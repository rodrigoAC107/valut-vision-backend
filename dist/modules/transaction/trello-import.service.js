...
exports.importFromTrelloCard = void 0;
...
const normalizeText = (value) => value
    ...
    .replace(/\s+/g, ' ')
    .toLowerCase();
const normalizeDescription = (value) => value.trim().replace(/\s+/g, ' ').toUpperCase();
const toTitleCase = (value) => normalizeText(value)
    .split(' ')
    .filter(Boolean)
    .map((word) => word.charAt(0).toUpperCase() + word.slice(1))
    .join(' ');
const getCategoryNameFor = (type, description) => {
    const normalizedDescription = normalizeText(description);
    const rule = CATEGORY_RULES.find((item) => item.type === type &&
...
const splitExplicitCategory = (line) => {
...
const stripTotals = (line) => line.replace(/\bTOTAL\b.*$/i, '').replace(/\*+/g, '').trim();
const extractDate = (line, year, defaultMonth) => {
...
const getAmountCandidates = (line) => Array.from(line.matchAll(/\b\d{1,3}(?:\.\d{3})+(?:,\d+)?\b|\b\d+(?:,\d+)?\b/g)).map((match) => ({
    raw: match[0],
    index: match.index ?? 0,
    amount: parseAmount(match[0]),
}));
const pickAmount = (line, amountFirst) => {
...
const removeAmountAt = (line, rawAmount, index) => `${line.slice(0, index)}${line.slice(index + rawAmount.length)}`.replace(/[()=*xX-]+/g, ' ').trim();
const parseAmount = (value) => Number(value.replace(/\./g, '').replace(',', '.'));
...
const buildSourceKey = (type, date, amount, description) => `${type}|${formatDateKey(date)}|${amount}|${normalizeDescription(description)}`;
const getYearFromCard = (cardName, description) => {
    const match = `${cardName}\n${description}`.match(/\b(20\d{2})\b/);
    return match ? Number(match[1]) : new Date().getFullYear();
};
const getMonthFromCard = (cardName, description) => {
...
const parseLine = (rawLine, currentType, year, defaultMonth) => {
    const line = rawLine.replace(/\\([.])/g, '$1').replace(/\u200c/g, '').trim();
    if (!line)
        return null;
    const header = normalizeText(line);
    if (SECTION_BY_HEADER[header] ||
...
const parseDescription = (card) => {
    ...
    let currentType = hasExpenseMarkers ? 'expense' : 'income';
...
const getPreferredListId = async (baseParams) => {
    ...
    const lists = await trelloRequest(`https://api.trello.com/1/boards/${TRELLO_BOARD_ID}/lists?${params.toString()}`);
    return lists.find((list) => !list.closed && normalizeText(list.name) === normalizeText(listName))?.id ?? null;
};
const getTrelloCardByName = async (cardName) => {
    ...
    const cards = await trelloRequest(`https://api.trello.com/1/boards/${TRELLO_BOARD_ID}/cards?${params.toString()}`);
    const normalizedName = normalizeText(cardName);
    const allMatches = cards.filter((card) => !card.closed &&
        normalizeText(card.name) === normalizedName);
    const matches = preferredListId
        ? allMatches.filter((card) => card.idList === preferredListId)
        : allMatches;
    if (!matches.length) {
        throw new Error(preferredListId
            ? `No se encontro una card activa llamada "${cardName}" en la lista GASTOS.`
            : `No se encontro una card activa llamada "${cardName}".`);
    }
    if (matches.length > 1) {
        return matches.sort((a, b) => new Date(b.dateLastActivity || 0).getTime() - new Date(a.dateLastActivity || 0).getTime())[0];
    }
    return matches[0];
};
const getCategoryId = async (type, name) => {
    const category = await category_model_1.Category.findOneAndUpdate({ name, type, isDeleted: false }, { $setOnInsert: { name, type, monthlyBudget: null, isDeleted: false } }, { upsert: true, new: true });
    return category._id;
};
const importFromTrelloCard = async (cardName) => {
    ...
    let skipped = 0;
    for (const item of parsed) {
        const duplicate = await transaction_model_1.Transaction.exists({
            isDeleted: false,
            $or: [
...
exports.importFromTrelloCard = importFromTrelloCard;
